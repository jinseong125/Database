...
SELECT *
FROM tbl_department
WHERE dept_location = "대구" AND dept_id >=3;  -- 논리 연산자: AND, OR
...
SELECT *
FROM tbl_department
WHERE dept_location IN("서울", "대구");   -- OR 연산의 대체 연산: IN 연산 (권장)
...
SELECT *
FROM tbl_department
WHERE dept_location IS NULL;  -- IS NULL (NULL 이다.)  IS NOT NULL (NULL 이 아니다.)
...
WHERE dept_name LIKE "영%";  -- %: 와일드카드(만능문자), 글자 수 제한 없음
                              ...
                              -- 부정 연산을 함께 사용할 땐 NOT LIKE 형식순으로 사용
...
SELECT dept_name, dept_location -- GROUP BY 절의 칼럼만 조회 가능 (dept_name은 칼럼 때문에 조회 실패)
FROM tbl_department
...
SELECT dept_location AS 부서위치,  -- 권장O
       COUNT(*) AS 부서수
FROM tbl_department
GROUP BY dept_location
HAVING COUNT(*) >= 2;
...
FROM tbl_department
GROUP BY dept_location
HAVING 부서수 >= 2;  -- HAVING 절에서 별칭 사용은 버전에 따라 다름(최신 버전은 지원하는 추세, 권장X)
...
SELECT *
FROM tbl_department
ORDER BY dept_location ASC, dept_name;
