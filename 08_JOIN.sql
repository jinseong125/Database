...
# 부서아이디, 부서명, 직원아이디, 직원명 Cross Join으로 조회하기
...
/*
  ...
  2. 테이블 크기, 인덱스 유무, 조건절의 선택도(조건 필터링 결과가 적을 것으로 예상되는 테이블을 DRive Table로 선정)
     등을 이용해서 DRiven Table을 선택합니다.
  3. 과거 DB는 FROM 절에 작성한 테이블을 Drive Table로 인식하기도 합니다.
*/
...
SELECT d.dept_id, d.dept_name, d.dept_location, e.emp_id, e.emp_name
FROM tbl_department d
JOIN tbl_employee e
